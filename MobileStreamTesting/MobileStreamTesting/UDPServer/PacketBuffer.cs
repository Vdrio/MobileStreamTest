...
namespace MobileStreamTesting.UDPServer
{
    public class PacketBuffer : IDisposable
    {
        ...
        public void WriteJpeg(byte[] input, int frame = -1)
        {
            if (input != null)
            {
                if (frame != -1)
                ...
                bufferList.AddRange(input);
                System.Diagnostics.Debug.WriteLine("Successfully added jpeg to packet");
            }
            else
            {
                System.Diagnostics.Debug.WriteLine("Unsuccessfully added jpeg to packet");
            }
            buffUpdate = true;
        }
        ...
        public ImageSource ReadJpeg(int length, bool peek = true)
        {
            if (buffUpdate)
            {
                readBuffer = bufferList.ToArray();
        ...
        public SKBitmap ReadJpegToBitmap(int length, bool peek = true)
        {
            if (buffUpdate)
            {
                readBuffer = bufferList.ToArray();
